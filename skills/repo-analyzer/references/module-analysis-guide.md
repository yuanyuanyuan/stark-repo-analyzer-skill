...
模块划分可以参考 `$WORK_DIR/drafts/01-graphify-map.md`，但 Graphify 只提供导航线索。每条关系都必须回到目标仓库源码验证后才能进入结论：`EXTRACTED` 关系记录文件和行号后核实，`INFERRED` 关系保留为“待验证”，`AMBIGUOUS` 关系只能进入风险或问题清单。若图谱和源码冲突，源码裁决，冲突记录到 `drafts/07-cross-validation.md`。目标仓库只读，模块草稿只能写入本次 `$WORK_DIR/drafts/` 的所有权文件。
...
运行时提供 Agent 工具时，阶段 6 必须为每个核心模块启动独立 subagent 并行分析。运行时不提供该能力时，必须在模块深度分析前暂停，说明能力缺失和耗时影响，并让用户明确选择继续顺序分析或停止。用户同意后，主 Agent 按相同模块边界顺序完成草稿，在 `drafts/03-plan.md` 标记 `parallelism: degraded`，并保持相同的覆盖率、交叉验证和报告质量要求；未获同意前不得开始顺序分析。
...
模块必须按业务职责和数据流划分，而不是照搬目录。每个 subagent 只处理已分配范围，用路径和行号证明核心流程，并把跨模块推断交给主 Agent 复核；覆盖率达标只是底线，最终还要让读者理解该模块为何这样设计、与全局如何协作。
