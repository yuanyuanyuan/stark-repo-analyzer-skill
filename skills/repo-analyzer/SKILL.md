...
6. **为核心模块写 Evidence Plan**：在启动 subagent 前，为每个核心模块写一个轻量 Markdown 计划，嵌入 `drafts/05-modules-plan.md`。Evidence Plan 是阅读和分析范围，不是架构结论，不要求 JSON schema、不要求 CLI、不要求自动生成。
   - 每个核心模块的 Evidence Plan 必须包含：模块、架构问题、候选文件或候选入口、必需证据类型、可能需要抽查的风险路径、预期输出到模块草稿的判断范围
...
2. 基于各模块 Evidence Matrix 对比模块角色、入口点、主流程、跨模块依赖、风险点和开放问题，识别跨模块矛盾、重复判断和证据缺口
3. 综合回答探索问题，识别跨模块设计模式
4. 验证全局关联：每个模块的分析是否都连接到了项目整体设计哲学
5. 写入 `drafts/07-cross-validation.md`，保留 Evidence Matrix 中尚未解决的开放问题、降级结论，以及仍待验证的跨模块推断
...
5. **Unsupported Claims 检查（最终报告前，流程级）**：
   ...
   5. 将检查结果简要写入 `drafts/08-insights.md` 或 `drafts/07-cross-validation.md` 的 unsupported claims 小节：已验证结论、降级项、仍开放的问题。本步骤不实现自动扫描、LLM judge、硬阻塞或 quality-gate-report。
6. **多源融合**：以阶段 5 设计的报告章节结构为骨架，从各草稿中抽取内容填充。同一概念在多个草稿中出现时，取最详细版本并补充其他版本独有信息。融合后消除所有"见草稿 X"、"详见附录"等跳转指示
   - **Evidence Matrix 对齐**：合成前先对比各模块 Evidence Matrix，把模块角色、入口、主流程、跨模块依赖、关键设计决策、风险点和开放问题作为融合索引。Evidence Matrix 中未解决的问题不得写成确定性结论；跨模块冲突必须先验证、降级或保留为开放问题。
...
当前工作流包含源码锚点、Evidence Plan、Markdown Evidence Matrix，以及最终报告前的 Unsupported Claims 流程级检查。它仍保持纯文档 skill 形态：不新增 CLI、不要求 JSON schema、不生成 `module-evidence/*.json`、不自动解析 Evidence Matrix，不实现自动断言扫描、LLM judge 或硬 quality gate。
