...
V1 保留本文件后续的参考分析流程、Why > What 方法和报告责任，只增加一个强制的 Graphify 结构证据侧车。正式分析前，Agent 必须先完成 `acceptance/doctor.sh preflight --target <repo> --work-dir <WORK_DIR> --json`，再运行：
...
本仓库同时提供确定性的控制面入口：`PYTHONPATH=src python -m stark_repo_analyzer.cli analyze <input> --work-dir <WORK_DIR> --mode standard` 负责阶段 1-5 的固定产物和 Agent 任务清单；Agent 完成并交叉验证 `drafts/06-module-*.md` 后，运行 `PYTHONPATH=src python -m stark_repo_analyzer.cli finalize --work-dir <WORK_DIR>` 完成阶段 8。`validate --complete` 是 finalization 的机器校验入口。
