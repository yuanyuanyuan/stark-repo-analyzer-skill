...
- 有**架构可视化**（Mermaid 图表）
- 所有核心结论有源码锚点（file:line），无锚点内容已降级为假设/开放问题
...
7. **规模参考汇总（可选）**：若需要，可将各模块的行数统计写入 `drafts/08-coverage.md` 作为规模参考（不放入最终报告）。这仅供诊断，不是质量信号。
8. 汇总生成最终报告（核心结论必须带源码锚点；无锚点内容已降级为假设/开放问题）。
