...
if [ "$phase" = "post-graph" ]; then
  ...
  if [ -f "$graph_json" ] && [ -f "$graph_report" ]; then
    if [ -n "$PYTHON_BIN" ]; then
      graph_result=$("$PYTHON_BIN" - "$graph_json" "$graph_report" "$target" <<'PY'
import json
import re
import sys
...
if node_sources + unlocated_symbol_nodes != len(nodes):
    errors.append("graph contains nodes with invalid or missing source references")
...
print("OK\t{}\t{}\t{}\t{}\t{}\t{}".format(
    len(nodes), len(links), node_sources, link_sources, extracted, unlocated_symbol_nodes
))
for error in errors:
    print("ERROR\t" + error)
PY
    ...
    if [ "$validator_rc" -ne 0 ]; then
      record_failure "$EXIT_BLOCKED" "Graphify graph validator could not run"
    fi
    while IFS=$'\t' read -r kind value rest; do
      if [ "$kind" = "ERROR" ]; then
        ...
        graph_nodes="$value"
        IFS=$'\t' read -r graph_links node_sources link_sources extracted unlocated_nodes <<< "$rest"
      fi
    done <<< "$graph_result"
...
export DOCTOR_GRAPH_LINK_SOURCES="$link_sources"
export DOCTOR_GRAPH_EXTRACTED="$extracted"
export DOCTOR_GRAPH_UNLOCATED_NODES="$unlocated_nodes"
...
result = {
    ...
    "graphify": {
        ...
        "health": {
            ...
            "node_source_references": int(os.environ.get("DOCTOR_GRAPH_NODE_SOURCES") or 0),
            "edge_source_references": int(os.environ.get("DOCTOR_GRAPH_LINK_SOURCES") or 0),
            "extracted_edges": int(os.environ.get("DOCTOR_GRAPH_EXTRACTED") or 0),
            "unlocated_symbol_nodes": int(os.environ.get("DOCTOR_GRAPH_UNLOCATED_NODES") or 0),
        } if os.environ["DOCTOR_PHASE"] == "post-graph" else None,
    },
...
PY
    ...
    printf '{"schema_version":1,"phase":"%s","status":{"code":%s},"target":"%s","work_dir":"%s","failures":["doctor JSON serializer unavailable"]}\n' "$phase" "$status" "$target" "$work_dir"
  fi
fi
