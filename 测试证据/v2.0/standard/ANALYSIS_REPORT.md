# v2.0 standard 架构分析草稿

## 架构问题与模式边界

标准模式覆盖核心模块和关键边界，补充配置、共享组件、工具链等次要模块抽样。

为什么采用关键单元覆盖率：v2.0 要验证的是架构判断是否有源码锚点和实质判断，而不是读取了多少源码行。当前 standard 模式已按预算回填 coverage-units.json。
...
标准模式在 quick 主路径之外补充了配置、共享组件和工具链的抽样覆盖。这个层级的价值不是穷尽所有实现，而是验证 src 的核心判断不会脱离外围约束：配置决定运行时入口，shared-components 影响交互一致性，tools 和 scripts 影响工程交付路径。

标准模式的替代方案检查集中在“是否把次要模块提升为 core”。本次保留其 secondary 分级，因为它们支撑主流程但不直接承载长截图切割的核心业务闭环。
