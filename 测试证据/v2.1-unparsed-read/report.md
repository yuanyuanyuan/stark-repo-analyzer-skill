# 长截图分割器 · 架构分析报告（v2.1-unparsed-read · standard）

> 目标仓：`/tmp/Long_screenshot_splitting_tool` @ `bdee20b8c4e4985c690a255ed09f64a3e335fd20`  
> 分析器：repo-analyzer skill v2 · 证据目录：`测试证据/v2.1-unparsed-read`  
> 模式：standard · **parallelism: degraded（主 Agent 串行）** · 已执行 Unparsed File Read Pass
...
仓库采用**扁平单仓**：`src` 主应用，`shared-components` 可复用 UI，`config` 配置聚合，`tools`/`scripts` 工程工具链。确定性扫描 `parse_rate` ≈ **48.2%**（ast-grep 对大量 TSX/入口解析不全），因此本报告对**可解析 hooks/utils/worker 主路径**给出较强结论；对 core unparsed 先执行 **manual-read 补读**，再统一标记 **Unsupported Area**。补读**不**抬高 parse_rate，也不把补读文件计为 analyzed unit。
...
5. 在解析器盲区下，如何对 App/上传/导出 UI 给出可审计但不伪装解析成功的结论？
...
- **偏好可持久化、二进制不持久化**。  
- **证据诚实**：enumerator 解析失败处用 manual-read 锚点补语义，同时保留 Unsupported。
...
证据锚点（parsed units）：`src/hooks/useAppState.ts:122`，`src/hooks/useImageProcessor.ts:19`，`src/hooks/useWorker.ts:42`，`src/workers/split.worker.js:75`，`src/utils/pdfExporter.ts:37`。

证据锚点（**manual-read / 非枚举单元**）：`src/main.tsx:5`，`src/App.tsx:28`，`src/App.tsx:196`，`src/components/FileUploader.tsx:22`，`src/components/ExportControls.tsx:129`。详见 `unparsed-file-reviews/`。

## 模块协作

跨模块关系以运行时组装为准：`src/App.tsx`（manual-read）引用 shared-components 展示件；`src/hooks/useImageProcessor.ts:19` 依赖 `src/hooks/useWorker.ts:23` 完成切图消息面；worker 在 `src/workers/split.worker.js:75` 路径调用 `src/utils/splitAnalyzer.ts` 纯函数分析；导出由 `src/utils/pdfExporter.ts:37` / `src/utils/zipExporter.ts:34` 消费 useAppState 切片集合。配置经 config 与 SEO loader 服务元数据，不进入 imageSlices 数据面。
...
`AppContent`（manual-read `src/App.tsx:28`）组装 hooks 与组件；切片首次从 0 变为 >0 才跳转 `/split`（manual-read `src/App.tsx:124-135`），避免 worker 未产出时被导航守卫踢回。

## 核心模块 src 深度

### 会话状态（useAppState）

Reducer 维护 worker、blobs、objectUrls、imageSlices、selectedSlices、isProcessing、splitHeight、fileName。`ADD_IMAGE_SLICE` **按 index 写入**（`src/hooks/useAppState.ts:47`）；`CLEANUP_SESSION` revoke URL 并 terminate worker，同时保留用户偏好（`src/hooks/useAppState.ts:89`）。正确性优先于“简单 push”：这是异步 onload 现实下的必要权衡。

### 切图编排（useImageProcessor + useWorker）

`processImage` 先 cleanup，再设 processing/fileName，创建原图预览，`createWorker` 后短暂等待并 `startProcessing(file, splitHeight)`（`src/hooks/useImageProcessor.ts:92`）。Worker 以 `type:'module'` 加载以支持 ESM import splitAnalyzer（`src/hooks/useWorker.ts:42`），消息类型 progress/chunk/done/error。chunk 路径创建 Object URL 并在 onload 后提交 slice——因此状态层必须抗乱序。

### 内容感知与 Worker（split.worker + splitAnalyzer）

Worker 流程：decode → 全图绘制 → `analyzeSplitPoints` → `computeSliceBounds` → JPEG 0.92 切片发送（`src/workers/split.worker.js:75`）。分析异常时 `splitPoints=[]` 强制回退等分。`splitAnalyzer` 以行级水平变化率找空白带，再页高驱动选点并做最小间距/末页合并；参数以 ratio 形式适配不同尺寸，与 DOM 解耦便于单测。

### 导出（pdfExporter / zipExporter）

两者都先 filter+sort 选中 index；空选择抛错（`src/utils/pdfExporter.ts:63`，`src/utils/zipExporter.ts:55`）。PDF 走 jsPDF 分页；ZIP 走 JSZip 打包下载。UI `ExportControls`（manual-read `src/components/ExportControls.tsx:129`）以 `canExport` 协同，体现算法与控件分离。

### Unparsed 主路径 UI（manual-read）

- **入口** `src/main.tsx:5`：只挂载 App。  
- **编排** `src/App.tsx:28+`：唯一主路径组装层；上传 `src/App.tsx:196`、导出 `src/App.tsx:226+`。  
- **上传 UI** `src/components/FileUploader.tsx:22`：校验后回调，不创建 Worker。  
- **备选编排** `src/components/ScreenshotSplitter.tsx:25`：内部再次 `useAppState`+`useImageProcessor`，存在双编排风险。  
- **预览/导航**：`ImagePreview`/`Navigation` 管选择与步骤，不持有 blob 源真值。

以上均为 **manual-read / 非枚举单元**，完整笔记见 `unparsed-file-reviews/*` 与 `module-evidence/src.json#unparsed_manual_reads`。

## 设计权衡

1. **Worker vs 主线程 canvas**：换流畅 UI，代价是协议与生命周期复杂度。动机是长图 decode/getImageData 昂贵；替代是同步主线程，卡顿更直接。  
2. **内容感知 vs 固定高度**：改善切口质量，但引入全图 `getImageData` 内存峰值与参数校准成本；用安全回退封底。  
3. **扁平单仓 vs monorepo package**：降低发布复杂度，边界靠约定。  
4. **解析器能力不足**：分析工具 parse_rate 低，迫使证据策略改为“主路径深读 + Unparsed File Read Pass + 组件区 Unsupported”。

## 风险、限制与 Unsupported Area

- **风险**：Object URL 泄漏；cleanup 后迟到 chunk；导出空选择；超长图全图像素分析 OOM；index 稀疏数组在 length 语义上的边角；App 与 ScreenshotSplitter 双编排。  
- **限制**：`parallelism: degraded`；parse_rate≈48%；refs 大量 partial/missing，跨模块结论不能写成已完全图验证；补读不改变 parse_rate。  
- **Unsupported Area**：下列 core 未解析文件不得当作已通过 enumerator 验证的实现细节（以 `coverage-units.json#unparsed` 为准）。已补读者额外给出 manual-read 锚点，但**仍声明 unsupported**：
...
**优点**：主路径职责清晰（状态 / Worker / 纯函数分析 / 导出分离）；乱序与资源清理有针对性设计；内容感知带安全回退体现产品工程成熟度；导出扩展点干净；文档（CLAUDE/ARCHITECTURE）与代码主线一致。

**不足**：组件与 SEO/调试能力相对“切割工具”核心偏重；`App.tsx` 体量大、编排集中；console 日志偏多；迟到消息无世代号；工具链 parse 差导致外部分析困难；导航/SEO/i18n 多 hook 抬高理解成本；ScreenshotSplitter 与 App 可能双源状态。
...
- 模式：standard  
- 并行：**parallelism: degraded**（主 agent 串行，实际 subagent=0）  
- Unparsed File Read Pass：**unparsed_read_pass.parallelism: degraded**；7 个高影响文件落盘 reviews + matrix.unparsed_manual_reads  
- 覆盖：core src ≥60% 关键单元；次要模块 ≥30% 抽样  
- Semantic Source Review：3 条（useAppState / useImageProcessor / worker processImage）  
- 外部调研：README + ARCHITECTURE + CLAUDE.md  
- 风险抽样：乱序 / 资源生命周期 / 空导出 / 内存峰值

## 开放问题

见 `module-evidence/src.json` 的 `open_questions`：组件层未解析细节、shared-components 边界、迟到 chunk、双编排挂载关系。

## Unsupported Area 声明（core 未解析文件，完整列表）

本轮 doctor/units 显示以下 **core 模块未解析文件**，本报告不对这些路径声明 enumerator 覆盖充分（补读另见 manual-read 锚点）：
