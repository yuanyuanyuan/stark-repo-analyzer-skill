...
普通用户不需要写 UAT 级长 prompt。只要能识别目标仓库，就按默认完整模式执行。
...
- 可选配置：脚本会读取 `~/.config/repo-analyzer/defaults.yaml`，支持 `extends:` 继承；CI 可用 `REPO_ANALYZER_MODE`、`REPO_ANALYZER_SLA_BUDGET_MINUTES`、`REPO_ANALYZER_TARGET_COVERAGE_CORE`、`REPO_ANALYZER_TARGET_COVERAGE_MINOR`、`REPO_ANALYZER_COVERAGE_ENGINE` 覆盖；需要复用上次偏好时加 `--use-last-pref`，需要保存本次偏好时加 `--save-pref`。
- 让脚本完成所有确定性动作：必要时浅克隆远程仓库、清空并重建 `OUTPUT`、枚举非二进制源码文件、识别仓库类型、生成切片、生成模块候选、生成受众报告、生成验收脚本和索引。
...
- 检查 `OUTPUT/SLA_REPORT.md` 和 `OUTPUT/CONFIG_EFFECTIVE.json`，确认本次 SLA 与生效配置已记录。
...
- 用户要核对可信度：读 `drafts/07-cross-ref-checks.md`、`coverage-symbols.json`、`SLA_REPORT.md` 和 `acceptance/check.sh` 输出。
...
- Coverage gate 默认 `auto`：本机有 `tree-sitter` CLI 时记录串行 parse 与大文件跳过统计；没有时退回确定性正则符号提取，不把可选依赖变成硬失败。
