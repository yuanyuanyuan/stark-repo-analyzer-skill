...
- 检查 `OUTPUT/08-coverage.md`、`OUTPUT/coverage-symbols.json` 和 `OUTPUT/STATE_REPORT.md`，确认覆盖率门控和状态报告已生成。
...
- `08-coverage.md` / `coverage-symbols.json`：符号覆盖率门控。
- `SLA_REPORT.md` / `CONFIG_EFFECTIVE.json`：SLA 与配置记录。
...
- 当前 coverage gate 使用确定性正则符号提取；需要 tree-sitter 精准 AST 覆盖时再升级。
