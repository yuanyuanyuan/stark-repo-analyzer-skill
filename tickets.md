...
- [ ] 风险路径抽样被描述为每个核心模块的必做流程，而不是可选旁注。
- [ ] 风险类别范围覆盖错误处理、配置、插件/扩展点、缓存、并发、权限与安全边界、generated/vendor/test 边界等。
- [ ] 每个抽样结果必须包含抽样对象、源码锚点、发现结果和影响说明。
- [ ] 不存在相关风险类别时必须说明为什么不适用。
- [ ] 明确 v1.5 不要求全覆盖、自动识别、安全扫描器或硬质量门。
...
- [ ] 核心模块 subagent prompt 要求执行至少一条相关风险路径抽样。
- [ ] 风险抽样结果进入 Evidence Matrix 或模块草稿专门小节。
- [ ] 风险抽样包含源码锚点，不能只写抽象风险标签。
- [ ] 不相关风险类别在草稿中给出不适用理由。
...
- [ ] 最终报告合成步骤要求汇总风险抽样结果。
- [ ] 最终报告的批判性评价引用风险抽样证据，而不是只复述 happy path。
- [ ] 风险抽样中的未验证推断仍受 unsupported claims 规则约束，不得被放大成确定性结论。
- [ ] 风险发现对架构评价的影响在最终报告中可见。
...
- [ ] 验收清单覆盖每个核心模块是否有风险抽样记录。
- [ ] 验收清单覆盖风险抽样是否包含源码锚点。
- [ ] 验收清单覆盖风险抽样是否影响最终报告评价。
- [ ] 验收清单覆盖不适用风险类别是否有理由说明。
- [ ] 用户文档说明报告会覆盖风险路径，而不只是主流程。
- [ ] 用户文档说明 v1.5 仍是文档规则，不引入自动风险扫描器。
...
- [ ] 每个模式声明 evidence 深度、风险抽样强度、报告长度、subagent 数量上限和外部调研强度。
- [ ] 明确 v1.6 不要求精确 token 统计或自动 token 中断。
- [ ] 明确 v1.6 继承 v1.1 到 v1.5 的全部行为。
...
- [ ] 模式选择影响 evidence 深度、风险抽样强度和报告长度，而不是只改名称。
- [ ] subagent prompt 或任务输入体现当前模式的证据读取预算。
- [ ] 每次分析记录预算目标和实际执行摘要。
- [ ] 超预算时缩小范围、降低深度或显式说明原因。
...
- [ ] 验收清单覆盖同仓三模式输出的相对差异。
- [ ] 验收清单检查快速模式是否减少 evidence 数量和报告长度。
- [ ] 验收清单检查标准模式是否保留核心模块和关键权衡。
- [ ] 验收清单检查深度模式是否增加风险路径和替代方案分析。
- [ ] 验收清单检查每次分析是否记录预算目标和实际执行摘要。
- [ ] 用户文档说明三模式差异由 evidence 深度和成本边界定义，而不是行覆盖率。
- [ ] 用户文档说明不提供精确 token 计费或自动中断。
