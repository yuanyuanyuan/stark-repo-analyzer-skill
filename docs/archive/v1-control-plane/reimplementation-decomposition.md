...
> **状态：已取代。** 当前执行顺序以
> [Graphify 精简执行计划](../../exec-plans/graphify-simplification-plan.md)为准。
> 本文仅保留为早期分解方法和历史阶段记录。
...
| D05 | 试做一个最小任务 | D04 | 一个可运行、可验证的 Graphify 闸门任务 | 无可用 LLM 或空图稳定失败；有效图谱任务连续两次成功，否则拆分任务 | 完成：doctor self-test、click 两次物理 Graphify 归档和 repeatability check 通过 |
