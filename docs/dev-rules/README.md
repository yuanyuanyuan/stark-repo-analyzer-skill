...
| [`workflows/`](workflows/) | Agent 作业流：任务类型入口、骨架映射、与 Judge/质量门衔接（按任务加载） |
| [`agent-boundaries/`](agent-boundaries/) | 硬规则/边界/暂停条件索引（不复制细则正文） |
...
2. **控制面机械护栏**（实现位于仓库工具与 hooks，规则解释在 `dual-agent-review/` / `document-control/`）：
   ...
   - Harness 导航：`python tools/release/validate-agent-harness.py`（不替代 Judge / control-plane audit）
