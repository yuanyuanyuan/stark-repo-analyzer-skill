...
- **fallback 为 `codex exec`**：运行时没有原生子代理时，Orchestrator 自动运行 [`tools/release/run-independent-judge.py`](../../../tools/release/run-independent-judge.py)，它以 `codex exec` 发起临时 Judge 并只返回审查区块。沙箱默认 `workspace-write`（仅允许验证侧写：临时目录 / pytest basetemp / 可忽略缓存），**角色仍只读**：禁止改正式文件、跑会改写正式树的 formatter、改 Git 状态。
...
- 至少独立执行一项与核心风险直接相关的验证，并在 `Judge Review` 写明**自己实际跑了什么**与结果。
- 代码任务优先**亲自重跑**针对性测试、lint、typecheck 或复现命令；文档任务检查 diff、链接、术语与规则符合性。
- **禁止用 Worker 自验证据顶替可本地重跑的检查。** 对单元测试、仓库内校验脚本、`git diff --check` 等廉价本地命令：Judge 必须独立重跑（或等价命令）；不得写「因沙箱/临时目录限制，采信 Worker 的 N passed」之类表述后给出 `pass`。
- 若环境导致必做的廉价重跑失败：先按本仓库 Judge 配置修正环境（见 fallback 沙箱与 `run-independent-judge.py`），仍无法执行则 `Verdict: blocked` 或 `revise`，在「缺失验证」中写清阻塞原因；**不得**因此降级为“审查 Worker 证据后 pass”。
- 下列高成本项**不**自动重复，可审查 Worker 的脱敏证据与退出状态：真实网络、付费 API、部署、不可逆操作、真实回归 UAT 全矩阵。廉价本地测试**不属于**本条豁免。
- 允许的验证侧写：系统临时目录、pytest `--basetemp`、gitignored 缓存（如 `.pytest_cache`）。禁止修改正式文件、控制面状态、Git 索引/历史，或运行会改写正式树的 formatter。
- 角色「只读」指不改交付物与仓库正式树，不是禁止一切磁盘写入；fallback 沙箱因此使用可验证侧写的 `workspace-write`，而不是以纯 `read-only` 为借口跳过重跑。
