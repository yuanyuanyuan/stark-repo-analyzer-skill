...
真实回归 UAT 证明**产品行为**；密钥/敏感资料扫描证明**仓库未把凭证带出站**。两者都是公开发版前的门，但证据与通过标准独立：

- 安全扫描规则：[pre-release-security-scan](../pre-release-security-scan/README.md)
- 不得用本文件的 UAT PASS 代替安全扫描；也不得用 gitleaks PASS 代替本文件的三条完整流程。
