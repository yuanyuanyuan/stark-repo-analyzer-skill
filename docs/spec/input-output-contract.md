...
3. Run Graphify `0.9.13+` as `graphify extract <target> --code-only --no-cluster --out <WORK_DIR>`, with no LLM/provider configuration. Then run Graphify's `cluster-only <WORK_DIR> --no-label --no-viz` to materialize the report. Preserve `raw-code-only-*` output and derive a source-locatable normalized graph/report under `$WORK_DIR/graphify-out/`.
