...
|---|---|---|
| 当前是否有实施主线 | [roadmap 索引](roadmap/README.md) → [exec plan 索引](exec-plans/README.md) | 当前无 `active`；最近完成 [Agent Harness 渐进披露](roadmap/agent-harness-progressive-disclosure-roadmap.md) |
...
| 用户分析场景导航 | [product-map](spec/product-map.md) | 选中场景行后打开合同/skill 链接 |
| Agent 作业流 / 硬边界 | [workflows](dev-rules/workflows/README.md) / [agent-boundaries](dev-rules/agent-boundaries/README.md) | 按任务类型加载，非每次通读 |
...
2. 从 roadmap 与 exec plan 目录索引确认是否存在 `active` 文档；当前无活动主线；最近完成含 Agent Harness 渐进披露、GitHub v1.1.x 与代码地图导航。
