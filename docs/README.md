...
|---|---|---|
| 当前是否有实施主线 | [roadmap 索引](roadmap/README.md) → [exec plan 索引](exec-plans/README.md) | 当前没有 `active` 文档；最近完成 [代码地图导航](roadmap/code-map-navigation-roadmap.md)；Graphify/Judge 等历史主线已完成 |
...
2. 从 roadmap 与 exec plan 目录索引确认是否存在 `active` 文档；当前没有活动实施主线；代码地图导航已 `completed`；Graphify/Judge 等记录已是 `completed`。
