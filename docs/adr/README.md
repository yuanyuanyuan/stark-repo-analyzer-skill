...
| 当前状态 | 活动主线见 roadmap 索引；ADR-0016 至 ADR-0028（含 0025 Skill 核心包分发与 0028 harness 渐进披露）描述已实现/本轮采用基线 |
...
| Skill 交付与安装 | [ADR-0025：以 Skill 核心交付包作为唯一分发真源](0025-use-the-skill-bundle-as-the-single-distribution-source.md)（skill-distribution 控制面已 `completed`；外部 marketplace 与 G5 真实回归不在原完成口径内） |
| 默认独立审查 | [ADR-0026：默认 Judge 使用范围受限的审查包](0026-default-judge-with-scoped-review-package.md) |
| 本仓库代码地图 | [ADR-0027：YAML 语义真源 + Graphify 只读协助 + 提醒 hook](0027-repo-code-map-with-graphify-assist-and-reminder-hooks.md) |
| Agent 导航脚手架 | [ADR-0028：分散补强渐进披露，不平行 ai-harness](0028-agent-harness-progressive-disclosure-without-parallel-ai-harness.md) |

## 已接受但尚未实施

当前无条目。Skill 核心包分发（ADR-0025）已随 skill-distribution 控制面落地，并列入上方已实现基线；「已落地」不等于真实外部 marketplace 安装或 G5 真实回归已验。

## 拟议决策

当前无拟议 ADR。

当前主线只采用上表中的 ADR-0016 至 ADR-0019、ADR-0021 至 ADR-0028（含 Skill 核心包分发 ADR-0025 与 Agent 导航脚手架 ADR-0028）。ADR-0001 至 ADR-0015 和 ADR-0020 默认都是历史输入；只有当前 ADR 或活动 roadmap 明确重新引入的部分，才能继续约束实现。
