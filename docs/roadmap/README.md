...
| 当前状态 | 当前没有 `active` roadmap；[skill-distribution-architecture-roadmap.md](skill-distribution-architecture-roadmap.md) 为 `proposed`，[graphify-simplification-roadmap.md](graphify-simplification-roadmap.md) 已 `completed` |
...
[`graphify-simplification-roadmap.md`](graphify-simplification-roadmap.md) 已完成 G0-G4 实施主线。当前没有活动 roadmap；新的跨多轮 initiative 必须先建立 `proposed` 文档并获得确认后再转为 `active`。被取代的目标文档保留在 `docs/archive/v1-control-plane/`，并带有明确的 `superseded` 标记。

## Proposed Roadmap

[`skill-distribution-architecture-roadmap.md`](skill-distribution-architecture-roadmap.md) 已记录 Claude/Codex 原子交付架构和四阶段迁移边界。它尚未获得实现授权，不能据此移动 gate、删除旧入口或修改当前产品合同。
