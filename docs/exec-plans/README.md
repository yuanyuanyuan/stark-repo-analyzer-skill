...
| 当前状态 | 当前没有 `active` plan；[skill-distribution-architecture-plan.md](skill-distribution-architecture-plan.md) 为 `proposed`，[graphify-simplification-plan.md](graphify-simplification-plan.md) 已 `completed` |
...
plan 可以使用 `proposed`、`active`、`blocked`、`completed` 或 `superseded`。只有被活动 roadmap 引用的 plan 才能标记为 `active`；完成时必须说明哪些验收已执行、哪些未执行。
...
[`graphify-simplification-plan.md`](graphify-simplification-plan.md) 已完成 G0-G4 必需任务。当前没有活动计划；发布触发 G5 或开始新的跨多轮工作时，必须新建或显式激活对应计划。被取代的任务清单和实现计划保留在 `docs/archive/v1-control-plane/`，不能覆盖当前合同。

## Proposed 计划

[`skill-distribution-architecture-plan.md`](skill-distribution-architecture-plan.md) 与
[`skill-distribution-architecture-progress.md`](skill-distribution-architecture-progress.md)
记录 Skill 原子交付迁移的未来顺序和已完成设计事实。plan 未激活前，任务表不构成实现授权。
...
- 状态：proposed | active | blocked | completed | superseded
- 日期：YYYY-MM-DD
